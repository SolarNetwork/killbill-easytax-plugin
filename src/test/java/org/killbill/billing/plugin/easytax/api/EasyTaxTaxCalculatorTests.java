...
public class EasyTaxTaxCalculatorTests {
    ...
    private void testComputeItemsOverTime(final PluginTaxCalculator calculator,
        ...
        assertBigDecimalEquals(taxation.getTotalTax(),
                taxableItem1.getAmount().add(taxableItem2.getAmount()).multiply(TEST_TAX_RATE), 2,
        ...
        List<EasyTaxTaxation> taxations6 = dao.getTaxation(tenantId, account.getId(),
                invoice.getId());
        assertEquals(taxations6.size(), 1, "Taxation record count");
        EasyTaxTaxation taxation6 = taxations6.get(0);
        assertEquals(taxation6.getKbTenantId(), tenantId, "Tenant ID");
        // TODO: verify values
